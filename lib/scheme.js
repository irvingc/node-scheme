...
var add = function(args) {
  ...
  if(args.type && args.type == Types.Cons) {
    ...
    if(args.car.type != Types.Number) {
      return new Error('All arguments must be numbers');
    }
    ...
    if(args.cdr.type == Types.Cons) {
      ...
      result.val += r.val;
      return result;
    } else if(args.cdr.type == Types.Null) {
      return result;
    } else {
      return new Error('Cannot work in improper lists');
    }

  } else {
    return new Error('Adding only works on lists of numbers');
  }
};

var adddef = new Scheme.Cons(new Scheme.Symbol('+'), new Scheme.Function(add));
...
function evalFunction(expr) {
  ...
  if(!Util.isError(fun)) {
    ...
    console.log("Function args", Util.format(args));
...
Scheme.eval = function(expr) {
  ...
  if(expr.type == Types.Cons) {
    ...
    switch(expr.car.type) {
     ...
     case Types.Cons:
        return new Error('Cons as car not implemented');
      default:
        return new Error('Semantic error, cannot eval');
    }
